...
const AddToCardBtn = ({
  ...
  const handleAddToCart = async () => {
    if (product) {
      ...
      // Also add to PHP backend cart
      try {
        const formData = new FormData()
        formData.append("product_id", product._id)
        formData.append("quantity", "1")

        await postFormToAPI("add_to_cart.php", formData)
      } catch (error) {
        console.error("Error adding to cart on backend:", error)
        // Continue anyway since we've added to local store
      }
  ...
  const handleDeleteProduct = async () => {
    if (existingProduct) {
      if (existingProduct?.quantity > 1) {
        ...
        // Also update PHP backend cart
        try {
          const formData = new FormData()
          formData.append("product_id", existingProduct._id)
          formData.append("action", "decrease")

          await postFormToAPI("update_cart.php", formData)
        } catch (error) {
          console.error("Error updating cart on backend:", error)
          // Continue anyway since we've updated local store
        }
